...
The Format command uses `FormatSettingsLoader.LoadAsync` with `SqlFormatterExtension.ExtensibilityInstance` and the active editor's managed document buffer. SSMS loads its effective settings and applies the document's `.editorconfig`. The bridge invokes `SqlFormatHelper.ToScriptGeneratorOptions`, `CreateParser`, and `CreateScriptGenerator`, including their non-public members, and passes the resulting ScriptDOM objects into Axial's existing post-processing pipeline.
...
4. Open a saved SQL file under an `.editorconfig` with SSMS SQL Formatter overrides. Compare native/Axial output. Repeat with a file under different overrides and an unsaved query to verify document-specific settings are not cached across windows.
...
7. Verify a parse error or unavailable/incompatible formatter leaves query text unchanged. Switch query windows while settings load and confirm formatting stops if the active buffer changes.
8. Smoke-test scripting/export features, which still use their standalone path.

The host adapter uses the shell's MEF service and the documented [GetDocumentBuffer API](https://learn.microsoft.com/en-us/dotnet/api/microsoft.visualstudio.editor.ivseditoradaptersfactoryservice.getdocumentbuffer?view=visualstudiosdk-2022) to obtain the buffer passed to SSMS's settings loader.

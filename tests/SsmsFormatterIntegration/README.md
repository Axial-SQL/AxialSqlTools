...
- The formatter has no compile-time assembly reference. Reuse its loaded assembly when available; otherwise load `Extensions/Microsoft/SSMS.SqlFormatter/Microsoft.SqlServer.Management.SqlFormatter.dll` relative to the running SSMS executable with `Assembly.LoadFrom`. This avoids normal probing-path failures and a build-time version pin. ComponentModelHost remains directly referenced with Copy Local disabled. No formatter DLL, decompiled SSMS implementation, editor SDK package, or new settings storage is distributed.

...
- [**Format Any TSQL Code**](https://github.com/Axial-SQL/AxialSqlTools/wiki/TSQL-Code-Formatting-with-Microsoft-ScriptDOM-library): Format SQL using the installed SSMS SQL Formatter settings and the active document's `.editorconfig`, then apply Axial's additional formatting options. Enable Disregard SSMS Formatter settings to use the original Axial code generation with fresh instances of the parser and generator types selected by SSMS. Shift+Format provides per-run overrides, including this setting. Requires an SSMS version with the built-in SQL Formatter.

...
- **Sort Selected Text**: Select comma-separated text, then right-click and choose **Sort Selected Text Ascending** or **Sort Selected Text Descending**. By default, the result is one horizontal list. Hold **Shift** while choosing either command to put each item on its own line, separated by commas. Items are trimmed; numeric lists sort numerically and other lists sort alphabetically without changing case. Every comma is treated as a separator. Available in the SQL editor context menu only.
